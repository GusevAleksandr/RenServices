...
object ContractListApi {
  ...
  /**
   * получение списка документов ранее переданными параметрами фильтрации. Параметры фильтрации передавались post-запросом на /rest-api/list/contract
   * 
   * Expected answers:
   *   code 200 : Seq[Contract] (массив доступных пользователю документов. Если заданы поля, необходимые для загрузки, или флаг onlyHeaderFields не выставлен в значение false возвращается массив объектов в виде пар ключ&#x3D;значение. Если параметр onlyHeaderFields выставлен в значение false, возвращается полное JSON представление документа)
   ...
   */
  def getContractList(id: String, page: Option[Int] = None, pageSize: Option[Int] = None)(implicit apiKey: ApiKeyValue): ApiRequest[Seq[Contract]] =
    ApiRequest[Seq[Contract]](ApiMethods.GET, "https://virtserver.swaggerhub.com/renessansBankService/restServices/1.0.0", "/rest-api/list/contract/{id}", "application/json")
  ...
  def postContractsList(postBody: DocListRequest)(implicit apiKey: ApiKeyValue): ApiRequest[String] =
    ApiRequest[String](ApiMethods.POST, "https://virtserver.swaggerhub.com/renessansBankService/restServices/1.0.0", "/rest-api/list/contract", "application/json")
      .withApiKey(apiKey, "Authorization", HEADER)
      .withBody(postBody)
